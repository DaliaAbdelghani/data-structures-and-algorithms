...
const toLastNames = people => {
  let newArr=[];
    people.map( element =>{
    newArr.push (`${element.firstName}' '${element.firstName}`);
    return newArr; 
  })
};
...
const hasNumber = (string) => {
  (string === /[A-Za-z][0-9]/g) ? true: false;
...
const validateEmail = (email) => {
  (email === /\w[0-9],\w[0-9]@\w.net|.com.|org/g) ? true: false;
};
