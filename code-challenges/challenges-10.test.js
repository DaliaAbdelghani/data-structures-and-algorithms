...
const getNames = (arr) => {
  let newArr = [];
  arr.forEach((element)=>{newArr.push(element.name)} )
  newArr.forEach((element)=>{ element.split("").reverse().join("")});
  return newArr;
};
...
const count = (target, input) => {
  ...
  if (element.includes(target)) { count=count+1 }  
  })
  return count;
};
...
const totalSum = (input) => {
  ...
  function reducer (accumulator, currentValue) { accumulator + currentValue;
