...
Write a function named findHappiness that takes in an array of strings and returns an array containing only
...
const findHappiness = (arr) => {
  const newArr = [];
  arr.forEach((element) => { if (element.includes(':)')) { newArr.push(element) } })
  return newArr;
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map((element) => {
    let newEle = element.replace(/[\s()-]/g, '')
    return newEle;
  })
};
